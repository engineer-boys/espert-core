#include "VulkanMiniRAPIContext.hh"
...
// helpful data type
using ExtContextData = esp::VulkanMiniRAPIContext::ContextData;

// signatures
static bool check_validation_layer_support(ExtContextData& context_data);
static std::vector<const char*>
get_required_extensions(ExtContextData& context_data);
static void populate_debug_messenger_create_info(
    VkDebugUtilsMessengerCreateInfoEXT& create_info);
static void has_glfw_required_instance_extensions(ExtContextData& context_data);
...
static bool is_device_suitable(VkPhysicalDevice device,
                               ExtContextData& context_data);
static esp::QueueFamilyIndices
find_queue_families(VkPhysicalDevice device, ExtContextData& context_data);
...
namespace esp
{
  VulkanMiniRAPIContext* VulkanMiniRAPIContext::s_instance = nullptr;

  VulkanMiniRAPIContext::VulkanMiniRAPIContext()
  {
    ESP_ASSERT(s_instance == nullptr, "The vulkan context already exists!");
    VulkanMiniRAPIContext::s_instance = this;

#ifdef __APPLE__
    m_context_data.m_instance_extensions.push_back(
  ...
  VulkanMiniRAPIContext::~VulkanMiniRAPIContext() { terminate(); }

  void VulkanMiniRAPIContext::init(EspWindow& window)
  {
  ...
  void VulkanMiniRAPIContext::terminate()
  {
    ESP_ASSERT(s_instance != nullptr,
               "You cannot terminate vulkan context because it doesn't exist!");

    vkDestroyDevice(m_context_data.m_device, nullptr);

    if (m_context_data.m_enable_validation_layers)
    {
      destroy_debug_utils_messenger_EXT(m_context_data.m_instance,
                                        m_context_data.m_debug_messenger,
                                        nullptr);
    }

    vkDestroySurfaceKHR(m_context_data.m_instance,
                        m_context_data.m_surface,
                        nullptr);
    vkDestroyInstance(m_context_data.m_instance, nullptr);
    VulkanMiniRAPIContext::s_instance = nullptr;
  }

  void VulkanMiniRAPIContext::update() {}

  std::unique_ptr<VulkanMiniRAPIContext> VulkanMiniRAPIContext::create()
  {
    return std::unique_ptr<VulkanMiniRAPIContext>{
      new VulkanMiniRAPIContext()
    };
  }

  const VulkanMiniRAPIContext::ContextData&
  VulkanMiniRAPIContext::get_context_data()
  {
    return s_instance->m_context_data;
  }

  /* ------------------ API DEPENDENT CODE ------------------- */

  void VulkanMiniRAPIContext::create_instance()
  {
    ...
    if (m_context_data.m_enable_validation_layers &&
        !check_validation_layer_support(m_context_data))
    {
    ...
    if (m_context_data.m_enable_validation_layers)
    {
      create_info.enabledLayerCount =
          static_cast<uint32_t>(m_context_data.m_validation_layers.size());
      create_info.ppEnabledLayerNames =
          m_context_data.m_validation_layers.data();
    ...
    if (vkCreateInstance(&create_info, nullptr, &(m_context_data.m_instance)) !=
        VK_SUCCESS)
    {
  ...
  void VulkanMiniRAPIContext::setup_debug_messenger()
  {
  ...
  void VulkanMiniRAPIContext::create_surface(EspWindow& window)
  {
    window.create_window_surface(m_context_data.m_instance,
                                 &m_context_data.m_surface);
  }

  void VulkanMiniRAPIContext::pick_physical_device()
  {
    uint32_t device_count = 0;
    vkEnumeratePhysicalDevices(m_context_data.m_instance,
                               &device_count,
                               nullptr);
    if (device_count == 0)
    {
    ...
    m_context_data.m_queue_family_indices =
        find_queue_families(m_context_data.m_physical_device, m_context_data);

    VkPhysicalDeviceProperties properties;
    vkGetPhysicalDeviceProperties(m_context_data.m_physical_device,
                                  &properties);
    ESP_CORE_INFO("Physical device: {0}", properties.deviceName);
  }

  void VulkanMiniRAPIContext::create_logical_device()
  {
    ...
    create_info.enabledExtensionCount =
        static_cast<uint32_t>(m_context_data.m_device_extensions.size());
    create_info.ppEnabledExtensionNames =
        m_context_data.m_device_extensions.data();
    ...
    if (vkCreateDevice(m_context_data.m_physical_device,
                       &create_info,
                       nullptr,
                       &m_context_data.m_device) != VK_SUCCESS)
    {
    ...
    vkGetDeviceQueue(m_context_data.m_device,
                     indices.m_graphics_family,
                     0,
                     &m_context_data.m_graphics_queue);
    vkGetDeviceQueue(m_context_data.m_device,
                     indices.m_present_family,
                     0,
                     &m_context_data.m_present_queue);
  }

  void VulkanMiniRAPIContext::create_buffer_command_object() {}

  void VulkanMiniRAPIContext::create_frame_scheduler() {}
                               ...
                               ExtContextData& context_data)
{
  ...
  bool extensions_supported =
      check_device_extension_support(device, context_data);
...
static esp::QueueFamilyIndices find_queue_families(VkPhysicalDevice device,
                                                   ExtContextData& context_data)
{
                                           ...
                                           ExtContextData& context_data)
{
  ...
  std::set<std::string> required_extensions(
      context_data.m_device_extensions.begin(),
