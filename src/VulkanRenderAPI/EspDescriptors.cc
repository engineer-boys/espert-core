...
namespace esp
{
  ...
  EspDescriptorSetLayout::Builder& EspDescriptorSetLayout::Builder::add_binding(uint32_t binding,
  ...
  EspDescriptorSetLayout::EspDescriptorSetLayout(EspDevice& device,
  ...
  void EspDescriptorSet::bind(VkCommandBuffer command_buffer, VkPipelineLayout pipeline_layout)
  {
  ...
  EspDescriptorPool::Builder& EspDescriptorPool::Builder::add_pool_size(VkDescriptorType descriptor_type,
                                                                        uint32_t count)
  {
  ...
  EspDescriptorPool::Builder& EspDescriptorPool::Builder::set_pool_flags(VkDescriptorPoolCreateFlags flags)
  {
    m_pool_flags = flags;
    return *this;
  }
  ...
  EspDescriptorPool::EspDescriptorPool(EspDevice& device,
                                       uint32_t max_sets,
  ...
  bool EspDescriptorPool::allocate_descriptor_set(const VkDescriptorSetLayout& descriptor_set_layout,
                                                  VkDescriptorSet& descriptor) const
  {
    ...
    if (vkAllocateDescriptorSets(m_device.get_device(), &alloc_info, &descriptor) != VK_SUCCESS) { return false; }
    return true;
  }
  ...
  void EspDescriptorPool::reset_pool() { vkResetDescriptorPool(m_device.get_device(), m_descriptor_pool, 0); }
  ...
  EspDescriptorWriter& EspDescriptorWriter::write_buffer(uint32_t binding, VkDescriptorBufferInfo* buffer_info)
  {
    ESP_ASSERT(m_set_layout.m_bindings.count(binding) == 1, "Layout does not contain specified binding")

    auto& binding_description = m_set_layout.m_bindings[binding];

    ESP_ASSERT(binding_description.descriptorCount == 1, "Binding single descriptor info, but binding expects multiple")
  ...
  EspDescriptorWriter& EspDescriptorWriter::write_image(uint32_t binding, VkDescriptorImageInfo* image_info)
  {
    ESP_ASSERT(m_set_layout.m_bindings.count(binding) == 1, "Layout does not contain specified binding")
  ...
  {
    bool success = m_pool.allocate_descriptor_set(m_set_layout.get_layout(), set.get_set());
