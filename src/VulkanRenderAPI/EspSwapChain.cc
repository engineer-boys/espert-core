...
namespace esp
{
  ...
  VkResult EspSwapChain::acquire_next_image(uint32_t* image_index)
  {
    ...
    VkResult result = vkAcquireNextImageKHR(m_device.get_device(),
                                            m_swap_chain,
                                            std::numeric_limits<uint64_t>::max(),
  ...
  VkResult EspSwapChain::submit_command_buffers(const VkCommandBuffer* buffers, uint32_t* image_index)
  {
  ...
  void EspSwapChain::create_image_views()
  {
    ...
    for (size_t i = 0; i < m_swap_chain_images.size(); i++)
    {
      ...
      if (vkCreateImageView(m_device.get_device(), &view_info, nullptr, &m_swap_chain_image_views[i]) != VK_SUCCESS)
      {
  ...
  void EspSwapChain::create_framebuffers()
  {
    ...
    for (size_t i = 0; i < image_count(); i++)
    {
      ...
      if (vkCreateFramebuffer(m_device.get_device(), &framebuffer_info, nullptr, &m_swap_chain_framebuffers[i]) !=
          VK_SUCCESS)
      {
  ...
  void EspSwapChain::create_sync_objects()
  {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
      if (vkCreateSemaphore(m_device.get_device(), &semaphore_info, nullptr, &m_image_available_semaphores[i]) !=
              VK_SUCCESS ||
          vkCreateSemaphore(m_device.get_device(), &semaphore_info, nullptr, &m_render_finished_semaphores[i]) !=
              VK_SUCCESS ||
  ...
  VkSurfaceFormatKHR
  EspSwapChain::choose_swap_chain_surface_format(const std::vector<VkSurfaceFormatKHR>& available_formats)
  {
  ...
  VkPresentModeKHR
  EspSwapChain::choose_swap_chain_present_mode(const std::vector<VkPresentModeKHR>& available_present_modes)
  {
