...
namespace esp
{
  ...
  std::unique_ptr<EspUniformManager> VulkanPipeline::create_uniform_manager() const
  {
    return std::unique_ptr<EspUniformManager>{ new VulkanUniformManager(*m_uniform_data, m_pipeline_layout) };
  }
} // namespace esp
